...
export default function Register() {
    ...
    const registerUser = (e) => {
        ...
        if(e.target.password.value === e.target.rpassword.value) {            
            fetch("http://localhost:3030/users?username=" + e.target.username.value)
                .then((response) => response.json())
                .then((json) => {
                    let userData = json.find(user => user.username === e.target.username.value);
                    if (!userData) {            
                        fetch("http://localhost:3030/users?email=" + e.target.email.value)
                            .then((response) => response.json())
                            .then((json) => {
                                let userData = json.find(user => user.email === e.target.email.value);
                                if (!userData) {
                                    var role = e.target.admin_request.checked ? "admin" : "customer";
                                    let newUser = {
        ...
        <Row>
            ...
            <Col>
                <div className="jumbotron">
                    ...
                    <p class="fs-4 fw-light mb-3">Please enter your account registration detsils</p>
                    <br />
                    <Form onSubmit={registerUser}>
                        <Container className="w-75">
                            ...
                            <Form.Check className="col-sm-4" type="switch" id="admin_request" name="admin_request" label="Request Admin access" />
                        </Container>
                        <Button type="submit" variant="primary" className="w-25 mt-4">Complete registration</Button>
                    </Form>
                    <p class="text-center mt-1">Have an account? <a href="/login">Sign In</a></p>
                </div>
            </Col>
            <Col sm="3">
